...
<html>
	<head>
		<title>LPL 2019 Summer Split</title>
		<link rel="stylesheet" href="home.css">
	</head>
	<body>
		<header>
			<h1 id = 'LUL'>LPL 2019 Summer Season</h1>
		...
		<?php
			...
			#query all teams 
			$LPL = "select T.TeamID, Wins, Losses, League from Teams T, Regions R where R.LeagueID = T.League and T.League = 'LPL' order by Wins desc";

			echo "<main class = 'container'>";
			
			#query to get teams from the LPL
			$LPLResults = mysqli_query($connect, $LPL);
			if(!$LPLResults)
				die("Could not query through the regions for some reason");
			
			if(mysqli_num_rows($LPLResults) > 0){
				#echo "\n\t\t\t<p>Viewing all LPL teams</p>";
				echo "\n\t\t<br><br>\n";
				echo "\n\t\t\t<table id='t04' border='1' align='center'>\n";
				echo "\t\t\t\t<thead>\n";
					echo "\t\t\t\t\t<tr>\n";
					echo "\t\t\t\t\t\t<th>Team</th>\n";
					echo "\t\t\t\t\t\t<th>Wins</th>\n";
				...
				while($row = mysqli_fetch_array($LPLResults)){
					echo "\t\t\t\t\t<tr>\n";
