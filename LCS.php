...
<html>
	<head>
		<title>LCS 2019 Summer Split</title>
		<link rel="stylesheet" href="home.css">
	</head>
	<body>
		<header>
			<h1 id = 'LUL'>LCS 2019 Summer Season</h1>
		...
		<?php
			...
			#listing out all LCS teams
			$LCS = "select T.TeamID, Wins, Losses, League from Teams T, Regions R where R.LeagueID = T.League and T.League = 'LCS' order by Wins desc";
			
			echo "<main class = 'container'>";
	
			#get results from query
			$LCSResults = mysqli_query($connect, $LCS);
			if(!$LCSResults)
				die("Could not query through the regions for some reason");
			
			#list out all the teams in the LCS
			if(mysqli_num_rows($LCSResults) > 0){
				#echo "\n\t\t\t<p>Viewing all LCS teams</p>";
				echo "\n\t\t\t<br><br>\n";
				echo "\t\t\t<table id='t01' border='1' align='center'>\n";
				echo "\t\t\t\t<thead>\n";
					echo "\t\t\t\t\t<tr>\n";
					echo "\t\t\t\t\t\t<th>Team</th>\n";
					echo "\t\t\t\t\t\t<th>Wins</th>\n";
				...
				while($row = mysqli_fetch_array($LCSResults)){
					echo "\t\t\t\t\t<tr>\n";
