...
<html>
	<head>
		<title>LEC 2019 Summer Split</title>
		<link rel="stylesheet" href="home.css">
	</head>
	<body>
		<header>
			<h1 id = 'LUL'>LEC 2019 Summer Season</h1>
		...
		<?php
			...
			#listing out all teams in LEC
			$LEC = "select T.TeamID, Wins, Losses, League from Teams T, Regions R where R.LeagueID = T.League and T.League = 'LEC' order by Wins desc";

			echo "<main class = 'container'>";
	
			#query through all teams that are in the LEC
			$LECResults = mysqli_query($connect, $LEC);
			if(!$LECResults)
				die("Could not query through the regions for some reason");
			
			#list out all teams from the LEC
			if(mysqli_num_rows($LECResults) > 0){
				#echo "\n\t\t\t<p>Viewing all LEC teams</p>";
				echo "\n\t\t\t<br><br>\n";
				echo "\t\t\t<table id='t02' border='1' align='center'>\n";
				echo "\t\t\t\t<thead>\n";
					echo "\t\t\t\t\t<tr>\n";
					echo "\t\t\t\t\t\t<th>Team</th>\n";
					echo "\t\t\t\t\t\t<th>Wins</th>\n";
				...
				while($row = mysqli_fetch_array($LECResults)){
					echo "\t\t\t\t\t<tr>\n";
